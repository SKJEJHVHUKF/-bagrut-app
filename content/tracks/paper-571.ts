...
export const TRACK_571: TrackTree = {
  ...
  topics: [
    ...
    },
    {
      ...
      //
      // איתי, 2026-09-08: ההיפוך. הזהויות היו שלב 1 ועברו לשלב 4, מיד אחרי
      // נוסחאות השטח. בלשונו: "אני רוצה ללמד את הבסיס — את כל הבסיס, את הסינוס
      // קוסינוס וטאנגנס, ואז את המשפטים סינוסים + קוסינוסים, ואז את נוסחאות
      // השטח, ואז רק אחרי זה ללמד זהויות". הזהויות אינן שער כניסה לנושא אלא
      // כלי שמפשט ביטויים שהתלמיד כבר יודע לייצר, ולכן הן באות אחרי שיש לו מה
      // לפשט. הקפסטון (שאלות משולבות ברמת בגרות) נשאר אחרון.
      //
      // ⚠️ מוסכמת האותיות היווניות (אלפא/בתא/גמא) ישבה בתוך trig-plane-basics
      // כי הוא היה הראשון. עם ההיפוך היא הועברה לראש trig-right-triangle,
      // אחרת $\gamma$ בנוסחת השטח מופיעה שני שלבים לפני שהיא מוגדרת.
      tiles: [
        // ---- רמה 1 --------------------------------------------------------
        {
          kind: 'ladder',
        ...
        // ---- רמה 4 --------------------------------------------------------
        {
          ...
          bullets: ['למה בכלל צריך זהויות ומה הן מפשטות', '$\\sin(180°-\\alpha)$, $\\cos(180°-\\alpha)$ וזהות פיתגורס', 'זווית כפולה: $\\sin(2\\alpha) = 2\\sin\\alpha\\cos\\alpha$', 'משוואה בתחום, כולל הצבת $t$'],
        },
        // ---- רמה 5 --------------------------------------------------------
        {
          kind: 'ladder',
