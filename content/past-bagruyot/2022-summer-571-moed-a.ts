 ...
 *
 * תוכן (מתווסף בהדרגה):
 *   שאלה 1 — ארבעה סעיפים עצמאיים: א) אינדוקציה (4^n-1 מתחלק ב-3, ומציאת p),
 *            ב) חקירת f(x)=(2-1/x)^3, ג) אינטגרל/שטח h(t)=∫f (גרף קווי),
 *            ד) גאומטריה (מעגל חסום במשולש DEF, ∠F=180-4α).
 *   שאלה 2 — סדרות הנדסיות: I (מנה 9r^2) ו-II (מנה q=3r); התכנסות,
 *            S_II=4/3·S_I → r=1/9, סכומים (זוגיים=12→a1=32, כפולות-5=48/121),
 *            ויחס איבר/סכום-שאחריו = (1-q)/q.
 */
...
export const bagrut2022Summer571MoedA: PastBagrutQuestion[] = [
    ...
    parts: [
      {
        label: 'א1',
        prompt:
          '**סעיף א — אינדוקציה.** הוכיחו כי הביטוי $4^n - 1$ מתחלק ב-$3$ ללא שארית לכל $n$ טבעי.',
        answer_type: 'proof',
        hints: [
      ...
      },
      {
        ...
        answer_type: 'number',
        hints: [
          'קשרו לסעיף הקודם: כתבו $4^{n+1} + p = 4\\,(4^n - 1) + (4 + p)$.',
          'החלק $4\\,(4^n - 1)$ כבר מתחלק ב-$12$ (כי $4^n - 1$ מתחלק ב-$3$). מה צריך $4 + p$ לקיים?',
        ],
        solution: {
          steps: [
            'נקשר את הביטוי לתוצאה מסעיף א1. נכתוב $\\;4^{n+1} + p = 4 \\cdot 4^n + p$.',
            'נוסיף ונחסר $4$: $\\;4 \\cdot 4^n - 4 + 4 + p = 4\\,(4^n - 1) + (4 + p)$.',
            'מסעיף א1 $4^n - 1 = 3m$, ולכן $\\;4\\,(4^n - 1) = 12m$ — מתחלק ב-$12$.',
      ...
      },
      {
        label: 'ב1',
        prompt: [
          '**סעיף ב — חקירת פונקצייה.** נתונה הפונקצייה $f(x) = \\left(2 - \\dfrac{1}{x}\\right)^3$.',
          '',
          'מצאו את תחום ההגדרה של $f(x)$ ואת משוואות האסימפטוטות שלה (האנכית והאופקית).',
        ].join('\n'),
      ...
      },
      {
        label: 'ג1',
        prompt: [
          '**סעיף ג — אינטגרל.** נתונה הפונקצייה $h(t) = \\displaystyle\\int_0^t f(x)\\,dx$, המוגדרת בתחום $0 \\le t \\le 7$, כאשר $f(x)$ נתונה בגרף הקווי שלפניכם (שטחים חיוביים ב-$0 < x < 3$, שליליים ב-$3 < x < 7$).',
          '',
          'חשבו את $h(0)$, את $h(3)$ ואת $h(5)$.',
        ].join('\n'),
        answer_type: 'number',
        diagrams: [
          {
      ...
      },
      {
        ...
        solution: {
          steps: [
            ...
            'ערך המקסימום חושב בסעיף ג1: $\\;h(3) = 7$, ולכן נקודת המקסימום היא $(3, 7)$.',
          ],
      ...
      },
      {
        label: 'ד1',
        prompt: [
          '**סעיף ד — גאומטריה.** במשולש $DEF$ חסום מעגל, המשיק לצלע $DE$ בנקודה $A$, לצלע $EF$ בנקודה $B$, ולצלע $DF$ בנקודה $C$. נתון כי משולש המגע $ABC$ שווה-שוקיים ($AB = AC$), וזווית הראש שלו היא $\\angle BAC = 2\\alpha$.',
          '',
          'הוכיחו כי $\\angle D = \\angle E = 2\\alpha$.',
        ].join('\n'),
        answer_type: 'proof',
        diagrams: [
          {
      ...
      },
      {
        ...
        answer_type: 'proof',
        hints: ['השתמשו בסכום הזוויות במשולש $DEF$ ובתוצאה מסעיף ד1.'],
        solution: {
          steps: [
            'סכום הזוויות במשולש $DEF$ שווה $180°$: $\\;\\angle D + \\angle E + \\angle F = 180°$.',
            'מסעיף ד1 $\\angle D = \\angle E = 2\\alpha$, ולכן $\\;2\\alpha + 2\\alpha + \\angle F = 180°$.',
  ...
  },
  {
    ...
    totalPoints: 25,
    context: [
      'נתונה סדרה הנדסית אין-סופית $I$: $\\;a_1,\\, a_2,\\, a_3,\\, \\ldots$, שמנתה $q_1 = 9r^2$ (כאשר $0 < r < \\tfrac{1}{3}$).',
      'בין כל שני איברים סמוכים בסדרה $I$ מכניסים איבר אחד, וכך נוצרת סדרה הנדסית $II$: $\\;b_1,\\, b_2,\\, b_3,\\, \\ldots$ שמנתה $q$ (סדרה יורדת של איברים חיוביים).',
      'מכיוון שהוכנס איבר אחד בין כל שני סמוכים, מתקיים $\\;b_1 = a_1$, $b_3 = a_2$, $b_5 = a_3$, וכן הלאה.',
    ].join('\n'),
    parts: [
      {
        label: 'א1',
        prompt: 'הראו כי הסדרה $I$ מתכנסת, ומצאו את המנה $q$ של הסדרה $II$ (באמצעות $r$).',
        answer_type: 'expression',
        hints: [
          'סדרה הנדסית אין-סופית מתכנסת כאשר $|q_1| < 1$. בדקו מה נותן התנאי $0 < r < \\tfrac{1}{3}$.',
          '$b_1 = a_1$ ו-$b_3 = a_2 = a_1 q_1$; מצד שני $b_3 = b_1 q^2$. השוו.',
        ],
        solution: {
          steps: [
            'התכנסות $I$: נתון $0 < r < \\tfrac{1}{3}$, ולכן $0 < r^2 < \\tfrac{1}{9}$, ומכאן $\\;0 < q_1 = 9r^2 < 1$.',
            'מכיוון ש-$|q_1| < 1$, הסדרה $I$ מתכנסת.',
            'קשר בין המנות: $\\;b_3 = a_2 = a_1 q_1$, ומצד שני $b_3 = b_1 q^2 = a_1 q^2$ (כי $b_1 = a_1$).',
            'משווים: $\\;a_1 q^2 = a_1 q_1$, ומכאן $q^2 = q_1 = 9r^2$.',
            'המנה חיובית (סדרה יורדת של איברים חיוביים), ולכן $\\;q = 3r$.',
          ],
          final_answer: 'הסדרה $I$ מתכנסת ($0 < q_1 = 9r^2 < 1$), ומנת הסדרה $II$ היא $\\;q = 3r$.',
        },
      },
      {
        label: 'א2',
        prompt: 'הראו כי גם הסדרה $II$ מתכנסת.',
        answer_type: 'proof',
        hints: ['הציבו $q = 3r$ ובדקו את התנאי $|q| < 1$ לפי $0 < r < \\tfrac{1}{3}$.'],
        solution: {
          steps: [
            'מנת הסדרה $II$ היא $q = 3r$, ונתון $0 < r < \\tfrac{1}{3}$.',
            'לכן $\\;0 < q = 3r < 3 \\cdot \\tfrac{1}{3} = 1$.',
            'מכיוון ש-$|q| < 1$, גם הסדרה $II$ מתכנסת.',
          ],
          final_answer: 'גם $II$ מתכנסת, כי $\\;0 < q = 3r < 1$.',
        },
      },
      {
        label: 'ב',
        prompt:
          'נתון כי סכום אינסוף איברי $II$ גדול פי $\\tfrac{4}{3}$ מסכום אינסוף איברי $I$, כלומר $S_{II} = \\tfrac{4}{3} S_I$. מצאו את $r$.',
        answer_type: 'number',
        hints: [
          'סכום סדרה הנדסית מתכנסת הוא האיבר הראשון חלקי $1$ פחות המנה: $\\;S_I = \\dfrac{a_1}{1 - q_1}$. זכרו $b_1 = a_1$.',
          'הציבו $q_1 = 9r^2$, $q = 3r$, וצמצמו את $a_1$. שימו לב ש-$1 - 9r^2 = (1 - 3r)(1 + 3r)$.',
        ],
        solution: {
          steps: [
            'סכומי הסדרות: $\\;S_I = \\dfrac{a_1}{1 - q_1}$ ו-$S_{II} = \\dfrac{b_1}{1 - q} = \\dfrac{a_1}{1 - q}$ (כי $b_1 = a_1$).',
            'מציבים בתנאי $S_{II} = \\tfrac{4}{3} S_I$: $\\;\\dfrac{a_1}{1 - q} = \\dfrac{4}{3} \\cdot \\dfrac{a_1}{1 - q_1}$.',
            'מצמצמים $a_1$ וכופלים במכנים: $\\;3(1 - q_1) = 4(1 - q)$.',
            'מציבים $q_1 = 9r^2$ ו-$q = 3r$: $\\;3(1 - 9r^2) = 4(1 - 3r)$.',
            'מפרקים $1 - 9r^2 = (1 - 3r)(1 + 3r)$: $\\;3(1 - 3r)(1 + 3r) = 4(1 - 3r)$.',
            'מחלקים ב-$(1 - 3r)$ (חיובי, כי $r < \\tfrac{1}{3}$): $\\;3(1 + 3r) = 4$.',
            'פותרים: $\\;3 + 9r = 4$, ומכאן $r = \\dfrac{1}{9}$.',
          ],
          final_answer: '$r = \\dfrac{1}{9}$ (ואז $q = 3r = \\dfrac{1}{3}$ ו-$q_1 = 9r^2 = \\dfrac{1}{9}$).',
        },
      },
      {
        label: 'ג',
        prompt: [
          'מכאן ואילך $r = \\tfrac{1}{9}$ (ולכן $q = \\tfrac{1}{3}$).',
          '',
          'נתון כי סכום כל האיברים שבמקומות הזוגיים בסדרה $II$ ($b_2 + b_4 + b_6 + \\ldots$) שווה $12$. מצאו את $a_1$.',
        ].join('\n'),
        answer_type: 'number',
        hints: [
          'האיברים במקומות הזוגיים $b_2, b_4, b_6, \\ldots$ יוצרים סדרה הנדסית שאיברה הראשון $b_2$ ומנתה $q^2$.',
          'חשבו את $\\dfrac{b_2}{1 - q^2}$, השוו ל-$12$, וזכרו $b_2 = b_1 q$ ו-$a_1 = b_1$.',
        ],
        solution: {
          steps: [
            'האיברים במקומות הזוגיים $b_2, b_4, b_6, \\ldots$ יוצרים סדרה הנדסית שאיברה הראשון $b_2$ ומנתה $q^2 = \\left(\\tfrac{1}{3}\\right)^2 = \\tfrac{1}{9}$.',
            'סכומה: $\\;\\dfrac{b_2}{1 - q^2} = \\dfrac{b_2}{1 - \\tfrac{1}{9}} = \\dfrac{b_2}{\\tfrac{8}{9}} = \\dfrac{9}{8} b_2$.',
            'משווים ל-$12$: $\\;\\dfrac{9}{8} b_2 = 12$, ומכאן $b_2 = \\dfrac{32}{3}$.',
            'מכיוון ש-$b_2 = b_1 q = b_1 \\cdot \\tfrac{1}{3}$: $\\;b_1 \\cdot \\tfrac{1}{3} = \\dfrac{32}{3}$, ולכן $b_1 = 32$.',
            'ומכיוון ש-$a_1 = b_1$: $\\;a_1 = 32$.',
          ],
          final_answer: '$a_1 = 32$.',
        },
      },
      {
        label: 'ד',
        prompt:
          'מצאו את סכום כל האיברים בסדרה $II$ שמקומם מתחלק ב-$5$ (כלומר $b_5 + b_{10} + b_{15} + \\ldots$).',
        answer_type: 'number',
        hints: [
          'גם הם סדרה הנדסית: איבר ראשון $b_5$ ומנה $q^5$.',
          '$b_5 = b_1 q^4$. חשבו את $\\dfrac{b_5}{1 - q^5}$.',
        ],
        solution: {
          steps: [
            'האיברים $b_5, b_{10}, b_{15}, \\ldots$ יוצרים סדרה הנדסית שאיברה הראשון $b_5$ ומנתה $q^5 = \\left(\\tfrac{1}{3}\\right)^5 = \\tfrac{1}{243}$.',
            'האיבר הראשון: $\\;b_5 = b_1 q^4 = 32 \\cdot \\left(\\tfrac{1}{3}\\right)^4 = \\dfrac{32}{81}$.',
            'הסכום: $\\;S = \\dfrac{b_5}{1 - q^5} = \\dfrac{\\tfrac{32}{81}}{1 - \\tfrac{1}{243}} = \\dfrac{\\tfrac{32}{81}}{\\tfrac{242}{243}}$.',
            'מפשטים: $\\;S = \\dfrac{32}{81} \\cdot \\dfrac{243}{242} = \\dfrac{32 \\cdot 3}{242} = \\dfrac{96}{242} = \\dfrac{48}{121}$.',
          ],
          final_answer: '$\\dfrac{48}{121}$.',
        },
      },
      {
        label: 'ה',
        prompt:
          'הוכיחו כי בסדרה הנדסית מתכנסת כלשהי, היחס בין איבר לבין סכום כל האיברים שאחריו אינו תלוי במיקום האיבר. מהו יחס זה בסדרה $II$?',
        answer_type: 'proof',
        hints: [
          'סמנו איבר כללי $c_n$ ומנה $q_c$. סכום כל האיברים שאחריו הוא סדרה הנדסית שאיברה הראשון $c_{n+1}$.',
          'חשבו את היחס בין $c_n$ לבין הסכום שאחריו, ופשטו.',
        ],
        solution: {
          steps: [
            'נסמן איבר כללי $c_n$ בסדרה הנדסית מתכנסת שמנתה $q_c$.',
            'סכום כל האיברים שאחריו: $\\;c_{n+1} + c_{n+2} + \\ldots = \\dfrac{c_{n+1}}{1 - q_c} = \\dfrac{c_n q_c}{1 - q_c}$.',
            'היחס המבוקש: $\\;\\dfrac{c_n}{\\;\\dfrac{c_n q_c}{1 - q_c}\\;} = \\dfrac{1 - q_c}{q_c}$.',
            'הביטוי $\\dfrac{1 - q_c}{q_c}$ אינו תלוי ב-$n$, ולכן היחס זהה לכל איבר בסדרה.',
            'בסדרה $II$ (מנה $q = \\tfrac{1}{3}$): $\\;\\dfrac{1 - \\tfrac{1}{3}}{\\tfrac{1}{3}} = \\dfrac{\\tfrac{2}{3}}{\\tfrac{1}{3}} = 2$.',
          ],
          final_answer: 'היחס הוא $\\dfrac{1 - q_c}{q_c}$ ואינו תלוי במיקום; בסדרה $II$ הוא שווה $\\;2$.',
        },
      },
    ],
    solutionSource: 'authored',
