    ...
    parts: [
      {
        ...
        solution: {
          steps: [
            '**נפרק ונצמצם:** $\\;\\dfrac{x^2-1}{(x+2)(x-1)} = \\dfrac{(x-1)(x+1)}{(x+2)(x-1)} = \\dfrac{x+1}{x+2}$, חוקי כאשר $x\\ne 1$.',
            '**נרשום את התנאים:** $x \\ne 1$ (התאפסות במכנה המקורי), $x \\ne -2$, וגם $\\dfrac{x+1}{x+2} > 0$ (תנאי ה-$\\ln$).',
            '**נפתור את אי-השוויון:** $\\dfrac{x+1}{x+2} > 0$ כאשר שני הגורמים באותו סימן — $x > -1$ או $x < -2$.',
      ...
      },
      {
        ...
        solution: {
          steps: [
            '**אסימפטוטה אנכית ב-$x=-2$:** כש-$x\\to -2^-$ מתקיים $\\dfrac{x+1}{x+2}\\to\\dfrac{-1}{0^-}=+\\infty$, ולכן $f\\to +\\infty$.',
            '**אסימפטוטה אנכית ב-$x=-1$:** כש-$x\\to -1^+$ מתקיים $\\dfrac{x+1}{x+2}\\to\\dfrac{0^+}{1}=0^+$, ולכן $f\\to -\\infty$.',
            '**$x=1$ אינה אסימפטוטה:** שם $\\dfrac{x+1}{x+2}\\to\\tfrac{2}{3}$ סופי — זהו חור (אי-הגדרה נקודתית), לא אסימפטוטה.',
            '**אסימפטוטה אופקית:** כש-$x\\to\\pm\\infty$ מתקיים $\\dfrac{x+1}{x+2}\\to 1$, ולכן $f\\to\\ln 1 = 0$ — אסימפטוטה $y=0$. ⬛',
          ],
          final_answer:
      ...
      },
      {
        ...
        answer_type: 'expression',
        hints: [
          'כתוב $f(x) = \\ln(x+1) - \\ln(x+2)$ (בתחום ההגדרה), וגזור.',
          'הנגזרת מצטמצמת ל-$f\'(x) = \\dfrac{1}{(x+1)(x+2)}$. בדוק את הסימן שלה בכל תחום ההגדרה.',
        ],
        solution: {
          steps: [
            '**נגזור** (לפי $f = \\ln(x+1) - \\ln(x+2)$): $\\;f\'(x) = \\dfrac{1}{x+1} - \\dfrac{1}{x+2} = \\dfrac{(x+2)-(x+1)}{(x+1)(x+2)} = \\dfrac{1}{(x+1)(x+2)}$.',
            '**נבדוק את הסימן בתחום ההגדרה:** עבור $x<-2$ שני הגורמים שליליים, ועבור $x>-1$ שניהם חיוביים — בשני המקרים $(x+1)(x+2) > 0$.',
            '**מסקנה:** $f\'(x) > 0$ בכל תחום ההגדרה, ולכן $f$ **עולה ממש** בכל ענף — אין תחומי ירידה ואין נקודות קיצון. ⬛',
          ],
          final_answer:
      ...
      },
      {
        ...
        solution: {
          steps: [
            '**נאסוף את המאפיינים:** תחום $(-\\infty,-2)\\cup(-1,1)\\cup(1,\\infty)$; אסימפטוטות $x=-2,\\,x=-1,\\,y=0$; $f$ עולה ממש בכל ענף (א3); חור ב-$x=1$ (ערך גבול $\\ln\\tfrac{2}{3}$); חיתוך עם ציר $y$ ב-$(0,-\\ln 2)$.',
            '**ענף שמאלי $(-\\infty,-2)$:** עולה מ-$0$ (ב-$-\\infty$) ל-$+\\infty$ (ב-$-2^-$) — כולו חיובי.',
            '**ענף ימני $(-1,1)\\cup(1,\\infty)$:** עולה מ-$-\\infty$ (ב-$-1^+$) ל-$0$ (ב-$+\\infty$), עובר $(0,-\\ln 2)$, עם חור ב-$x=1$ — כולו שלילי. ⬛',
          ],
          final_answer:
      ...
      },
      {
        ...
        solution: {
          steps: [
            '**הדרישה:** $g=\\ln(f)$ מוגדרת כאשר $f(x)>0$, כלומר $\\ln\\dfrac{x+1}{x+2}>0 \\iff \\dfrac{x+1}{x+2}>1$.',
            '**נפתור:** $\\;\\dfrac{x+1}{x+2}-1 = \\dfrac{-1}{x+2} > 0 \\iff x+2 < 0 \\iff x < -2$.',
          ],
      ...
      },
      {
        ...
        solution: {
          steps: [
            '**נגזור הרכבה:** $\\;g\'(x) = \\dfrac{f\'(x)}{f(x)}$.',
            '**נבדוק סימן בתחום $x<-2$:** מהגדרת $g$ מתקיים $f(x)>0$, ומסעיף א3 $\\;f\'(x)=\\dfrac{1}{(x+1)(x+2)}>0$.',
            '**מסקנה:** $g\'=\\dfrac{(+)}{(+)}>0$ בכל התחום, ולכן $g$ **עולה ממש** ב-$(-\\infty,-2)$ — אין ירידה ואין קיצון. ⬛',
          ],
      ...
      },
      {
        ...
        solution: {
          steps: [
            '**תחום ומונוטוניות:** $g$ מוגדרת על $(-\\infty,-2)$ ועולה ממש שם (סעיף ב2).',
            '**גבולות:** כש-$x\\to-\\infty$: $f\\to 0^+$ ולכן $g=\\ln f\\to-\\infty$; כש-$x\\to -2^-$: $f\\to+\\infty$ ולכן $g\\to+\\infty$.',
            '**חיתוך עם ציר $x$:** $g(x)=0 \\iff f(x)=1 \\iff \\dfrac{x+1}{x+2}=e \\iff x = \\dfrac{1-2e}{e-1} \\approx -2.58$.',
            '**מסקנה:** עקומה עולה ממש מ-$-\\infty$ ל-$+\\infty$, עם אסימפטוטה אנכית $x=-2$, החוצה את ציר $x$ ב-$x\\approx-2.58$. ⬛',
          ],
          final_answer:
      ...
      },
      {
        ...
        solution: {
          steps: [
            '**סימן $f$:** נתון $0<f(x)<1$, ולכן $f>0$.',
            '**סימן $g$:** $g=\\ln(f)$, ו-$f$ בתחום $(0,1)$ — לוגריתם של מספר כזה שלילי, אז $g<0$.',
            '**מסקנה:** $f\\cdot g = (+)\\cdot(-) < 0$ — המכפלה **שלילית**, כלומר אינה חיובית. ⬛',
          ],
          final_answer:
