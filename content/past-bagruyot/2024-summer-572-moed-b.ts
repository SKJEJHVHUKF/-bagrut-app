 ...
 *   Q4 — חקירת פונקציות מעריכיות: k(x)=xe^x, m(x)=2e^x-1 ו-f(x)=(e^x-1)/(x-1) —
 *        אסימפטוטות, מונוטוניות, הקשר f'=(k-m)/(x-1)^2, ושתי נקודות החיתוך c,d.
 *   Q5 — חקירת פונקציית ln: f(x)=(2lnx-1)/x, הפונקצייה הקדומה g(x)=ln^2 x - ln x,
 *        ו-h(x)=1+a/g(x) — קיצון, חיתוכי צירים, ותחום, וזיהוי הגרף (II).
 */
...
export const bagrut2024Summer572MoedB: PastBagrutQuestion[] = [
  ...
  },
  {
    ...
    totalPoints: 25,
    context: [
      'נתונות שתי פונקציות, המוגדרות לכל $x$: $\\;k(x) = xe^x\\;$ וגם $\\;m(x) = 2e^x - 1$.',
      'נתון כי גרפי $k(x)$ ו-$m(x)$ נחתכים בשתי נקודות בדיוק — אחת שבה $x = c$ ואחת שבה $x = d$ (כאשר $c < d$).',
      'בסעיפים ג–ו תיבדק גם הפונקצייה $f(x) = \\dfrac{e^x - 1}{x - 1}$, המוגדרת לכל $x \\ne 1$.',
    ].join('\n'),
    parts: [
      {
        label: 'א1',
        prompt:
          'מצאו את משוואות האסימפטוטות המקבילות לציר ה-$x$ (המאונכות לציר $y$) של כל אחת מן הפונקציות $k(x)$ ו-$m(x)$.',
        answer_type: 'expression',
        hints: [
          'אסימפטוטה מאוזנת נמצאת מן הגבולות ב-$x \\to +\\infty$ וב-$x \\to -\\infty$.',
          'עבור $k(x) = xe^x$: ב-$-\\infty$ המעריכית "מנצחת" והמכפלה שואפת ל-$0$; ב-$+\\infty$ הפונקצייה שואפת ל-$\\infty$.',
          'עבור $m(x) = 2e^x - 1$: ב-$-\\infty$ המחובר $2e^x \\to 0$, ולכן $m \\to -1$.',
        ],
        solution: {
          steps: [
            'עבור $k(x) = xe^x$ ב-$+\\infty$: $\\;\\lim_{x \\to \\infty} xe^x = \\infty$ (אין אסימפטוטה מאוזנת בכיוון זה).',
            'ב-$-\\infty$ המעריכית שואפת ל-$0$ מהר יותר מ-$x$: $\\;\\lim_{x \\to -\\infty} xe^x = 0$.',
            'לכן ל-$k(x)$ יש אסימפטוטה מאוזנת $\\;y = 0$ (כאשר $x \\to -\\infty$).',
            'עבור $m(x) = 2e^x - 1$ ב-$+\\infty$: $\\;\\lim_{x \\to \\infty}(2e^x - 1) = \\infty$ (אין אסימפטוטה).',
            'ב-$-\\infty$: $\\;\\lim_{x \\to -\\infty}(2e^x - 1) = 2 \\cdot 0 - 1 = -1$.',
            'לכן ל-$m(x)$ יש אסימפטוטה מאוזנת $\\;y = -1$ (כאשר $x \\to -\\infty$).',
          ],
          final_answer:
            'אסימפטוטה מאוזנת של $k$: $\\;y = 0$; של $m$: $\\;y = -1$ (לשתיהן רק בכיוון $x \\to -\\infty$).',
        },
      },
      {
        label: 'א2',
        prompt:
          'מצאו את תחומי העלייה והירידה של כל אחת מן הפונקציות $k(x)$ ו-$m(x)$ (אם יש כאלה).',
        answer_type: 'expression',
        hints: [
          'גזרו כל פונקצייה. $e^x > 0$ תמיד, ולכן הסימן של הנגזרת נקבע ע"י הגורם השני.',
          'עבור $k$: $k\'(x) = e^x(x + 1)$ — הסימן כמו של $x + 1$.',
          'עבור $m$: $m\'(x) = 2e^x > 0$ לכל $x$.',
        ],
        solution: {
          steps: [
            'גוזרים את $k(x) = xe^x$ בכלל המכפלה: $\\;k\'(x) = e^x + xe^x = e^x(x + 1)$.',
            'מכיוון ש-$e^x > 0$ תמיד, הסימן של $k\'$ נקבע ע"י $x + 1$; מאפסים: $x = -1$.',
            'עבור $x < -1$: $\\;k\'(x) < 0$ — הפונקצייה יורדת.',
            'עבור $x > -1$: $\\;k\'(x) > 0$ — הפונקצייה עולה.',
            'ב-$x = -1$ יש מינימום: $\\;k(-1) = -e^{-1} = -\\dfrac{1}{e}$.',
            'גוזרים את $m(x) = 2e^x - 1$: $\\;m\'(x) = 2e^x$.',
            'מכיוון ש-$2e^x > 0$ לכל $x$, הפונקצייה $m$ עולה תמיד (אין תחום ירידה).',
          ],
          final_answer:
            '$k(x)$: יורדת ב-$x < -1$ ועולה ב-$x > -1$ (מינימום ב-$\\left(-1,\\, -\\tfrac{1}{e}\\right)$). $m(x)$: עולה לכל $x$.',
        },
      },
      {
        label: 'א3',
        prompt:
          'מצאו את שיעורי נקודת החיתוך של כל אחד מן הגרפים של $k(x)$ ו-$m(x)$ עם ציר ה-$y$.',
        answer_type: 'expression',
        hints: ['נקודת חיתוך עם ציר $y$ מתקבלת בהצבת $x = 0$.'],
        solution: {
          steps: [
            'חיתוך עם ציר $y$ — מציבים $x = 0$.',
            'עבור $k$: $\\;k(0) = 0 \\cdot e^0 = 0$, כלומר הנקודה $(0, 0)$.',
            'עבור $m$: $\\;m(0) = 2e^0 - 1 = 2 - 1 = 1$, כלומר הנקודה $(0, 1)$.',
          ],
          final_answer: '$k$ חותכת את ציר $y$ ב-$(0, 0)$; $\\;m$ חותכת ב-$(0, 1)$.',
        },
      },
      {
        label: 'ב',
        prompt: 'סרטטו באותה מערכת צירים סקיצה של גרף $k(x)$ ושל גרף $m(x)$.',
        answer_type: 'text',
        diagrams: [
          {
            type: 'functionGraph',
            ...
            yRange: [-2, 12],
            curves: [
              { fn: (x) => x * Math.exp(x), color: '#2563EB' },
              { fn: (x) => 2 * Math.exp(x) - 1, color: '#059669' },
            ],
            hAsymptotes: [
              { y: 0, label: 'y=0 (k)' },
              { y: -1, label: 'y=-1 (m)' },
            ...
            markedPoints: [
              { x: -1, y: -1 / Math.E, label: '(-1,-1/e)' },
              { x: 0, y: 1, label: '(0,1)' },
              { x: -1.146, y: -0.364, label: 'c' },
              { x: 1.841, y: 11.6, label: 'd' },
            ],
            caption:
              'הגרפים $k(x) = xe^x$ (כחול — מינימום $\\left(-1, -\\tfrac{1}{e}\\right)$, אסימפטוטה $y = 0$) ו-$m(x) = 2e^x - 1$ (ירוק — עולה, אסימפטוטה $y = -1$). הם נחתכים בשתי נקודות: $x = c$ (ב-$c < -1$) ו-$x = d$ (ב-$d > 1$).',
          },
        ],
        hints: [
          'השתמשו בממצאים: אסימפטוטות ($y = 0$ ל-$k$, $y = -1$ ל-$m$), מינימום $k$ ב-$\\left(-1, -\\tfrac{1}{e}\\right)$, חיתוכי ציר $y$, ושתי נקודות החיתוך $c$ ו-$d$.',
        ],
        solution: {
          steps: [
            '$k(x)$ יורדת עד המינימום $\\left(-1, -\\tfrac{1}{e}\\right)$ ואז עולה; מתקרבת ל-$y = 0$ מלמטה כאשר $x \\to -\\infty$, ועוברת דרך $(0, 0)$.',
            '$m(x)$ עולה תמיד; מתקרבת ל-$y = -1$ כאשר $x \\to -\\infty$, ועוברת דרך $(0, 1)$.',
            'שני הגרפים נחתכים בשתי נקודות: $x = c$ (ב-$c < -1$) ו-$x = d$ (ב-$d > 1$).',
          ],
          final_answer:
            'ראו סקיצה: $k$ עם מינימום ב-$\\left(-1, -\\tfrac{1}{e}\\right)$ ואסימפטוטה $y = 0$; $m$ עולה עם אסימפטוטה $y = -1$; הגרפים נחתכים ב-$x = c$ וב-$x = d$.',
        },
      },
      {
        label: 'ג1',
        prompt:
          'מצאו את משוואות האסימפטוטות המאונכות לצירים של הפונקצייה $f(x) = \\dfrac{e^x - 1}{x - 1}$.',
        answer_type: 'expression',
        hints: [
          'אסימפטוטה אנכית — במקום שבו המכנה מתאפס אך המונה אינו מתאפס.',
          'אסימפטוטה מאוזנת — מן הגבולות ב-$\\pm\\infty$. ב-$-\\infty$ המונה $\\to -1$ והמכנה $\\to -\\infty$.',
        ],
        solution: {
          steps: [
            'אסימפטוטה אנכית: המכנה מתאפס ב-$\\;x - 1 = 0$, כלומר $x = 1$.',
            'שם המונה $e^1 - 1 = e - 1 \\ne 0$, ולכן $\\;x = 1$ היא אסימפטוטה אנכית.',
            'ב-$+\\infty$: $\\;\\lim_{x \\to \\infty} \\dfrac{e^x - 1}{x - 1} = \\infty$ (המעריכית גוברת) — אין אסימפטוטה מאוזנת בכיוון זה.',
            'ב-$-\\infty$: המונה $e^x - 1 \\to -1$ והמכנה $x - 1 \\to -\\infty$, ולכן $\\;\\lim_{x \\to -\\infty} f(x) = 0$.',
            'לכן $\\;y = 0$ היא אסימפטוטה מאוזנת (כאשר $x \\to -\\infty$).',
          ],
          final_answer: 'אסימפטוטה אנכית: $\\;x = 1$. אסימפטוטה מאוזנת: $\\;y = 0$ (ב-$x \\to -\\infty$).',
        },
      },
      {
        label: 'ג2',
        prompt: 'מצאו את שיעורי נקודת החיתוך של גרף $f(x)$ עם ציר ה-$x$.',
        answer_type: 'expression',
        hints: ['חיתוך עם ציר $x$: $f(x) = 0$, כלומר המונה מתאפס.'],
        solution: {
          steps: [
            'חיתוך עם ציר $x$: מאפסים את הפונקצייה, $\\;f(x) = 0$.',
            'שבר מתאפס כאשר המונה מתאפס: $\\;e^x - 1 = 0$.',
            'פותרים: $\\;e^x = 1 \\Rightarrow x = 0$ (והמכנה שם $0 - 1 = -1 \\ne 0$).',
            'לכן נקודת החיתוך היא $(0, 0)$.',
          ],
          final_answer: 'גרף $f$ חותך את ציר $x$ בנקודה $(0, 0)$.',
        },
      },
      {
        label: 'ד',
        prompt: 'הסבירו מדוע למשוואה $f\'(x) = 0$ יש בדיוק שני פתרונות.',
        answer_type: 'proof',
        hints: [
          'גזרו את $f$ בכלל המנה, ופשטו את המונה — הוא יתקשר ל-$k(x)$ ו-$m(x)$.',
          'תקבלו $f\'(x) = \\dfrac{k(x) - m(x)}{(x - 1)^2}$; שבר מתאפס כאשר המונה מתאפס.',
          '$k(x) = m(x)$ בדיוק בשתי נקודות ($x = c$, $x = d$), ושתיהן שונות מ-$1$.',
        ],
        solution: {
          steps: [
            'גוזרים את $f(x) = \\dfrac{e^x - 1}{x - 1}$ בכלל המנה: $\\;f\'(x) = \\dfrac{e^x(x - 1) - (e^x - 1) \\cdot 1}{(x - 1)^2}$.',
          ...
          final_answer:
            'כי $\\;f\'(x) = \\dfrac{k(x) - m(x)}{(x - 1)^2}$, המתאפס בדיוק כאשר $k(x) = m(x)$ — מה שקורה בדיוק בשתי נקודות ($x = c$, $x = d$), ושתיהן $\\ne 1$.',
        },
      },
      {
        label: 'ה1',
        prompt: 'הסבירו מדוע $d > 1$.',
        answer_type: 'proof',
        hints: [
          'בדקו את ערכי $k$ ו-$m$ בנקודה $x = 1$.',
          '$k(1) = e$ ו-$m(1) = 2e - 1$. מי גדול יותר?',
          'ב-$x = 1$ הגרף של $m$ מעל $k$; הם נפגשים שוב (ב-$d$) רק כאשר $k$ עוקף — לכן $d > 1$.',
        ],
        solution: {
          steps: [
            'נציב $x = 1$: $\\;k(1) = 1 \\cdot e^1 = e \\approx 2.72$.',
            'וגם $\\;m(1) = 2e^1 - 1 = 2e - 1 \\approx 4.44$.',
            'מכאן $m(1) > k(1)$ — כלומר ב-$x = 1$ הגרף של $m$ נמצא מעל הגרף של $k$.',
            'עבור ערכי $x$ גדולים $k(x) = xe^x$ גדל מהר יותר מ-$m(x)$, ולכן בסופו של דבר $k$ עוקף את $m$.',
            'נקודת החיתוך $d$ (הגדולה מבין השתיים) היא המקום שבו $k$ עוקף את $m$; מכיוון שב-$x = 1$ עדיין $m > k$, ההצטלבות מתרחשת רק ב-$x > 1$.',
            'לכן $\\;d > 1$.',
          ],
          final_answer:
            'מכיוון ש-$k(1) = e < 2e - 1 = m(1)$, ב-$x = 1$ הגרף של $m$ עדיין מעל $k$; ההצטלבות $d$ (שבה $k$ עוקף את $m$) מתרחשת רק ב-$x > 1$, ולכן $d > 1$.',
        },
      },
      {
        label: 'ה2',
        prompt: 'הביעו באמצעות $c$ ו-$d$ את תחומי העלייה והירידה של הפונקצייה $f(x)$.',
        answer_type: 'expression',
        hints: [
          'מסעיף ד: $f\'(x) = \\dfrac{k(x) - m(x)}{(x - 1)^2}$. המכנה חיובי תמיד, ולכן הסימן של $f\'$ כמו של $k - m$.',
          'מן הסקיצה בסעיף ב: $k > m$ עבור $x < c$ ועבור $x > d$, ו-$m > k$ עבור $c < x < d$.',
          'זכרו ש-$f$ אינה מוגדרת ב-$x = 1$, ו-$c < 1 < d$ — לכן תחום הירידה מתפצל.',
        ],
        solution: {
          steps: [
            'מסעיף ד: $\\;f\'(x) = \\dfrac{k(x) - m(x)}{(x - 1)^2}$.',
            'המכנה $(x - 1)^2 > 0$ לכל $x \\ne 1$, ולכן הסימן של $f\'$ זהה לסימן של $k(x) - m(x)$.',
            'מן הסקיצה: $k > m$ (כלומר $f\' > 0$) עבור $x < c$ ועבור $x > d$.',
            'ו-$m > k$ (כלומר $f\' < 0$) עבור $c < x < d$.',
            'אך $f$ אינה מוגדרת ב-$x = 1$, וכיוון ש-$c < 1 < d$, תחום הירידה מתפצל לשניים.',
            'לכן $f$ עולה ב-$x < c$ וב-$x > d$, ויורדת ב-$c < x < 1$ וב-$1 < x < d$.',
          ],
          final_answer:
            'עלייה: $\\;x < c$ וגם $x > d$. ירידה: $\\;c < x < 1$ וגם $1 < x < d$ (הנקודה $x = 1$ מוחרגת — אסימפטוטה אנכית).',
        },
      },
      {
        label: 'ו',
        prompt: 'סרטטו סקיצה של גרף הפונקצייה $f(x)$.',
        answer_type: 'text',
        diagrams: [
          {
            type: 'functionGraph',
            ...
            yRange: [-6, 9],
            curves: [
              { fn: (x) => (Math.exp(x) - 1) / (x - 1), domain: [-3.5, 0.93], color: '#7C3AED' },
              { fn: (x) => (Math.exp(x) - 1) / (x - 1), domain: [1.07, 3.2], color: '#7C3AED' },
            ],
            vAsymptotes: [{ x: 1, label: 'x=1' }],
            hAsymptotes: [{ y: 0, label: 'y=0' }],
            markedPoints: [
              { x: -1.146, y: 0.318, label: 'max (x=c)' },
              { x: 1.841, y: 6.305, label: 'min (x=d)' },
              { x: 0, y: 0, label: '(0,0)' },
            ],
            caption:
              'הגרף $f(x) = \\dfrac{e^x - 1}{x - 1}$: אסימפטוטה אנכית $x = 1$ ומאוזנת $y = 0$. מקסימום ב-$x = c$, חיתוך ב-$(0, 0)$, ומינימום ב-$x = d$.',
          },
        ],
        hints: [
          'השתמשו באסימפטוטות ($x = 1$, $y = 0$), בנקודת החיתוך $(0, 0)$, במקסימום ב-$x = c$ ובמינימום ב-$x = d$.',
        ],
        solution: {
          steps: [
            'אסימפטוטות: אנכית $x = 1$ ומאוזנת $y = 0$ (ב-$-\\infty$).',
            'ענף שמאלי ($x < 1$): מתקרב ל-$y = 0$ מלמעלה ב-$-\\infty$, עולה עד מקסימום ב-$x = c$, יורד דרך $(0, 0)$, ושואף ל-$-\\infty$ כאשר $x \\to 1^-$.',
            'ענף ימני ($x > 1$): יורד מ-$+\\infty$ (כאשר $x \\to 1^+$) עד מינימום ב-$x = d$, ואז עולה ל-$+\\infty$.',
          ],
          final_answer:
            'ראו סקיצה: אסימפטוטות $x = 1$ ו-$y = 0$, מקסימום ב-$x = c$, מינימום ב-$x = d$, וחיתוך ב-$(0, 0)$.',
        },
      },
    ],
    solutionSource: 'authored',
  },
  {
    ...
    totalPoints: 25,
    context: [
      'בסרטוט שלפניכם מתואר גרף הפונקצייה $f(x) = \\dfrac{2\\ln x - 1}{x}$, המוגדרת בתחום $x > 0$.',
      'הפונקצייה $g(x)$ מוגדרת אף היא בתחום $x > 0$ ומקיימת $g\'(x) = f(x)$. שיעור ה-$y$ של נקודת הקיצון של $g(x)$ הוא $-\\tfrac{1}{4}$.',
      'הפונקצייה $h(x)$ מוגדרת כך: $h(x) = 1 + \\dfrac{a}{g(x)}$, כאשר $a$ פרמטר הגדול מ-$\\tfrac{1}{4}$.',
    ].join('\n'),
    diagrams: [
      {
        type: 'functionGraph',
        xRange: [0.25, 14],
        yRange: [-4, 1.1],
        curves: [{ fn: (x) => (2 * Math.log(x) - 1) / x, color: '#2563EB' }],
        hAsymptotes: [{ y: 0, label: 'y=0' }],
        markedPoints: [{ x: Math.sqrt(Math.E), y: 0, label: '(√e,0)' }],
        caption:
          'הפונקצייה $f(x) = \\dfrac{2\\ln x - 1}{x}$ (בתחום $x > 0$): חותכת את ציר $x$ ב-$\\sqrt{e}$, שואפת ל-$-\\infty$ כאשר $x \\to 0^+$ ומתקרבת ל-$0$ (מלמעלה) כאשר $x \\to \\infty$.',
      },
    ],
    parts: [
      {
        label: 'א',
        prompt: 'מצאו את שיעורי נקודת החיתוך של גרף הפונקצייה $f(x)$ עם ציר ה-$x$.',
        answer_type: 'expression',
        hints: ['חיתוך עם ציר $x$: $f(x) = 0$, כלומר המונה מתאפס.'],
        solution: {
          steps: [
            'חיתוך עם ציר $x$: מאפסים את הפונקצייה, $\\;f(x) = 0$.',
            'שבר מתאפס כאשר המונה מתאפס: $\\;2\\ln x - 1 = 0$.',
            'מבודדים: $\\;\\ln x = \\dfrac{1}{2}$.',
            'פותרים (בתחום $x > 0$): $\\;x = e^{1/2} = \\sqrt{e}$.',
          ],
          final_answer: 'נקודת החיתוך עם ציר $x$: $\\;\\left(\\sqrt{e},\\, 0\\right)$.',
        },
      },
      {
        label: 'ב1',
        prompt: 'מצאו את הפונקצייה $g(x)$.',
        answer_type: 'expression',
        hints: [
          '$g$ היא פונקצייה קדומה של $f$: $\\;g(x) = \\displaystyle\\int \\dfrac{2\\ln x - 1}{x}\\,dx$.',
          'הציבו $t = 2\\ln x - 1$ (אז $dt = \\tfrac{2}{x}\\,dx$), או זהו את הצורה $\\int t\\,dt$.',
          'קבעו את קבוע האינטגרציה מהתנאי: בנקודת הקיצון ($x = \\sqrt{e}$) הערך הוא $-\\tfrac{1}{4}$.',
        ],
        solution: {
          steps: [
            '$g$ היא פונקצייה קדומה של $f$: $\\;g(x) = \\displaystyle\\int \\dfrac{2\\ln x - 1}{x}\\,dx$.',
            'נציב $t = 2\\ln x - 1$, ואז $\\;dt = \\dfrac{2}{x}\\,dx$, כלומר $\\dfrac{1}{x}\\,dx = \\dfrac{1}{2}\\,dt$.',
            'לכן $\\;g(x) = \\displaystyle\\int t \\cdot \\dfrac{1}{2}\\,dt = \\dfrac{1}{2} \\cdot \\dfrac{t^2}{2} + C = \\dfrac{t^2}{4} + C$.',
            'מחזירים את $t$: $\\;g(x) = \\dfrac{(2\\ln x - 1)^2}{4} + C$.',
            'נקודת הקיצון של $g$ היא במקום שבו $g\' = f = 0$, כלומר ב-$x = \\sqrt{e}$.',
            'הערך שם: $\\;g(\\sqrt{e}) = \\dfrac{\\left(2 \\cdot \\tfrac{1}{2} - 1\\right)^2}{4} + C = 0 + C = C$.',
            'נתון $g(\\sqrt{e}) = -\\tfrac{1}{4}$, ולכן $\\;C = -\\dfrac{1}{4}$.',
            'מציבים ומפשטים: $\\;g(x) = \\dfrac{(2\\ln x - 1)^2 - 1}{4} = \\dfrac{4\\ln^2 x - 4\\ln x}{4}$.',
            'לכן $\\;g(x) = \\ln^2 x - \\ln x$.',
          ],
          final_answer: '$g(x) = \\ln^2 x - \\ln x$ (כלומר $\\ln x\\,(\\ln x - 1)$).',
        },
      },
      {
        label: 'ב2',
        prompt: 'מצאו את שיעורי נקודות החיתוך של גרף הפונקצייה $g(x)$ עם ציר ה-$x$.',
        answer_type: 'expression',
        hints: ['הציבו $g(x) = 0$ והוציאו $\\ln x$ כגורם משותף.'],
        solution: {
          steps: [
            'מציבים $\\;g(x) = 0$: $\\;\\ln^2 x - \\ln x = 0$.',
            'מוציאים גורם משותף: $\\;\\ln x\\,(\\ln x - 1) = 0$.',
            'אפשרות ראשונה: $\\;\\ln x = 0 \\Rightarrow x = 1$.',
            'אפשרות שנייה: $\\;\\ln x = 1 \\Rightarrow x = e$.',
          ],
          final_answer: 'נקודות החיתוך עם ציר $x$: $\\;(1, 0)$ ו-$(e, 0)$.',
        },
      },
      {
        label: 'ב3',
        prompt: 'סרטטו סקיצה של גרף הפונקצייה $g(x)$.',
        answer_type: 'text',
        diagrams: [
          {
            type: 'functionGraph',
            xRange: [0.35, 6],
            yRange: [-0.6, 3],
            curves: [{ fn: (x) => Math.log(x) ** 2 - Math.log(x), color: '#059669' }],
            markedPoints: [
              { x: Math.sqrt(Math.E), y: -0.25, label: 'min (√e,-1/4)' },
              { x: 1, y: 0, label: '(1,0)' },
              { x: Math.E, y: 0, label: '(e,0)' },
            ],
            caption:
              'הפונקצייה $g(x) = \\ln^2 x - \\ln x$: מינימום ב-$\\left(\\sqrt{e}, -\\tfrac{1}{4}\\right)$, אפסים ב-$(1, 0)$ ו-$(e, 0)$, ושאיפה ל-$+\\infty$ בשני הקצוות.',
          },
        ],
        hints: [
          'השתמשו: מינימום ב-$\\left(\\sqrt{e}, -\\tfrac{1}{4}\\right)$, חיתוכי ציר $x$ ב-$(1, 0)$ ו-$(e, 0)$, וההתנהגות בקצוות.',
        ],
        solution: {
          steps: [
            'נקודת הקיצון של $g$ היא ב-$x = \\sqrt{e}$; זהו מינימום (הנגזרת $g\' = f$ עוברת ממינוס לפלוס), עם ערך $g(\\sqrt{e}) = -\\tfrac{1}{4}$.',
            'חיתוכי ציר $x$: $\\;(1, 0)$ ו-$(e, 0)$; שימו לב $1 < \\sqrt{e} < e$, כלומר המינימום נמצא בין שני האפסים.',
            'בקצוות: $\\;\\lim_{x \\to 0^+} g(x) = +\\infty$ וגם $\\lim_{x \\to \\infty} g(x) = +\\infty$ (המחובר $\\ln^2 x$ גובר).',
          ],
          final_answer:
            'ראו סקיצה: מינימום ב-$\\left(\\sqrt{e}, -\\tfrac{1}{4}\\right)$, אפסים ב-$(1, 0)$ ו-$(e, 0)$, ושאיפה ל-$+\\infty$ בשני הקצוות.',
        },
      },
      {
        label: 'ג1',
        prompt: 'מצאו את תחום ההגדרה של הפונקצייה $h(x)$.',
        answer_type: 'expression',
        hints: ['$h(x) = 1 + \\dfrac{a}{g(x)}$ מוגדרת כאשר $g(x) \\ne 0$ (ובתחום $x > 0$).'],
        solution: {
          steps: [
            'הפונקצייה $h(x) = 1 + \\dfrac{a}{g(x)}$ מוגדרת רק כאשר המכנה אינו מתאפס, $\\;g(x) \\ne 0$ (ובתחום $x > 0$).',
            'מסעיף ב2, $\\;g(x) = 0$ ב-$x = 1$ וב-$x = e$.',
            'לכן יש להוציא נקודות אלו מתחום ההגדרה.',
          ],
          final_answer: 'תחום ההגדרה: $\\;x > 0$, $\\;x \\ne 1$, $\\;x \\ne e$.',
        },
      },
      {
        label: 'ג2',
        prompt: 'קבעו אם גרף הפונקצייה $h(x)$ חותך את ציר ה-$x$. נמקו את קביעתכם.',
        answer_type: 'proof',
        hints: [
          'הציבו $h(x) = 0$ ובודדו את $g(x)$.',
          'זכרו שהמינימום של $g$ הוא $-\\tfrac{1}{4}$, ולכן $g(x) \\ge -\\tfrac{1}{4}$. השוו לערך הדרוש.',
        ],
        solution: {
          steps: [
            'נבדוק אם קיים $x$ שבו $\\;h(x) = 0$: $\\;1 + \\dfrac{a}{g(x)} = 0$.',
            'מבודדים: $\\;\\dfrac{a}{g(x)} = -1$, ומכאן $g(x) = -a$.',
            'נתון $a > \\tfrac{1}{4}$, ולכן הערך הדרוש מקיים $\\;-a < -\\tfrac{1}{4}$.',
            'אבל ערך המינימום של $g$ הוא $-\\tfrac{1}{4}$, ולכן $\\;g(x) \\ge -\\tfrac{1}{4}$ לכל $x$ בתחום.',
            '$g(x)$ לעולם אינו קטן מ-$-\\tfrac{1}{4}$, ולכן המשוואה $g(x) = -a$ חסרת פתרון.',
          ],
          final_answer:
            'הגרף אינו חותך את ציר $x$: המשוואה $h(x) = 0$ דורשת $g(x) = -a < -\\tfrac{1}{4}$, אך $g(x) \\ge -\\tfrac{1}{4}$ תמיד.',
        },
      },
      {
        label: 'ד',
        prompt:
          'מצאו את שיעורי נקודת הקיצון של הפונקצייה $h(x)$, וקבעו את סוגה. הביעו את תשובתכם באמצעות $a$, אם יש צורך.',
        answer_type: 'expression',
        hints: [
          'גזרו: $h(x) = 1 + a\\,[g(x)]^{-1}$, ולכן $h\'(x) = \\dfrac{-a\\,g\'(x)}{[g(x)]^2}$.',
          'הנגזרת מתאפסת כאשר $g\'(x) = 0$, כלומר $f(x) = 0$ — זה קורה ב-$x = \\sqrt{e}$.',
          'לקביעת הסוג: המכנה חיובי ו-$a > 0$, ולכן הסימן של $h\'$ הפוך לסימן של $g\'$.',
        ],
        solution: {
          steps: [
            'גוזרים את $h(x) = 1 + a\\,[g(x)]^{-1}$ בכלל השרשרת: $\\;h\'(x) = -a\\,[g(x)]^{-2} \\cdot g\'(x) = \\dfrac{-a\\,g\'(x)}{[g(x)]^2}$.',
            'המכנה $[g(x)]^2 > 0$ ו-$a > 0$, ולכן $h\'(x) = 0$ רק כאשר $\\;g\'(x) = 0$.',
            'אך $g\'(x) = f(x)$, ו-$f(x) = 0$ ב-$x = \\sqrt{e}$; לכן נקודת הקיצון של $h$ היא ב-$x = \\sqrt{e}$.',
            'הערך: $\\;h(\\sqrt{e}) = 1 + \\dfrac{a}{g(\\sqrt{e})} = 1 + \\dfrac{a}{-\\tfrac{1}{4}} = 1 - 4a$.',
            'קביעת הסוג: הסימן של $h\'$ הפוך לסימן של $g\'$. בקטע $(1, e)$ שלפני $\\sqrt{e}$ מתקיים $g\' < 0$, ולכן $h\' > 0$; אחרי $\\sqrt{e}$ מתקיים $g\' > 0$, ולכן $h\' < 0$.',
            '$h$ עולה ואז יורדת סביב $x = \\sqrt{e}$, ולכן זו נקודת מקסימום.',
          ],
          final_answer:
            'נקודת מקסימום ב-$\\left(\\sqrt{e},\\, 1 - 4a\\right)$ (וכיוון ש-$a > \\tfrac{1}{4}$, הערך $1 - 4a < 0$).',
        },
      },
      {
        label: 'ה',
        prompt: 'קבעו איזה מן הגרפים I–IV שלפניכם מתאר את הפונקצייה $h(x)$.',
        answer_type: 'text',
        diagrams: [
          {
            type: 'functionGraph',
            ...
            yRange: [-8, 8],
            curves: [
              { fn: (x) => 1 + 1 / (Math.log(x) ** 2 - Math.log(x)), domain: [0.16, 0.95], color: '#7C3AED' },
              { fn: (x) => 1 + 1 / (Math.log(x) ** 2 - Math.log(x)), domain: [1.05, 2.66], color: '#7C3AED' },
              { fn: (x) => 1 + 1 / (Math.log(x) ** 2 - Math.log(x)), domain: [2.78, 7], color: '#7C3AED' },
            ],
            vAsymptotes: [
              { x: 1, label: 'x=1' },
              { x: Math.E, label: 'x=e' },
            ...
            markedPoints: [{ x: Math.sqrt(Math.E), y: -3, label: 'max (√e, 1-4a)' }],
            caption:
              'כך נראה גרף $h(x) = 1 + \\dfrac{a}{g(x)}$ (בשרטוט לדוגמה $a = 1$): אסימפטוטה מאוזנת $y = 1$, אסימפטוטות אנכיות $x = 1$ ו-$x = e$, שני ענפים חיצוניים מעל $y = 1$, וענף אמצעי כולו מתחת לציר $x$ (מקסימום $\\left(\\sqrt{e}, 1 - 4a\\right)$ שלילי). זהו גרף II.',
          },
        ],
        hints: [
          'רכזו את המאפיינים: אסימפטוטה מאוזנת, אסימפטוטות אנכיות, וסימן הפונקצייה בכל קטע.',
          'ב-$x \\to 0^+$ וב-$x \\to \\infty$: $g \\to +\\infty$, ולכן $h \\to 1$. אסימפטוטות אנכיות ב-$x = 1$ וב-$x = e$.',
          'בקטע $(1, e)$ הפונקצייה $h$ כולה מתחת לציר $x$ (המקסימום שם, $1 - 4a$, שלילי).',
        ],
        solution: {
          steps: [
            'אסימפטוטה מאוזנת: כאשר $x \\to 0^+$ וכאשר $x \\to \\infty$ מתקיים $g(x) \\to +\\infty$, ולכן $\\dfrac{a}{g(x)} \\to 0$ ו-$h(x) \\to 1$ — אסימפטוטה $y = 1$.',
            'אסימפטוטות אנכיות: ב-$x = 1$ וב-$x = e$ (שם $g = 0$).',
            'בקטעים החיצוניים ($0 < x < 1$ וגם $x > e$) מתקיים $g > 0$, ולכן $h > 1$ — הענפים מעל האסימפטוטה $y = 1$.',
            'בקטע האמצעי ($1 < x < e$) מתקיים $g < 0$, והמקסימום $h(\\sqrt{e}) = 1 - 4a < 0$ — כל הענף מתחת לציר $x$.',
            'הגרף המתאים לכל אלה — שני ענפים חיצוניים מעל $y = 1$ וענף אמצעי (דמוי גבעה) כולו מתחת לציר $x$ — הוא גרף II.',
          ],
          final_answer: 'הפונקצייה $h(x)$ מתוארת בגרף $\\;\\mathrm{II}$.',
        },
      },
    ],
    solutionSource: 'authored',
