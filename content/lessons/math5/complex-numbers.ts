...
🎯 **בבגרות:** "מצא את הקבוצה $|z - (1+i)| = 2$." → מעגל עם מרכז $(1, 1)$ ורדיוס 2.`,
  ...
  summary: [
    '**אלגברית** $z = a + bi$ • כפל $(a+bi)(c+di) = (ac-bd) + (ad+bc)i$',
    '**חילוק בצמוד** $\\dfrac{z_1}{z_2} = \\dfrac{z_1 \\bar{z_2}}{|z_2|^2}$',
    '**מודול ארגומנט** $|z| = \\sqrt{a^2+b^2}$ • $\\theta = \\arctan(b/a)$ + תיקון רבע',
    '**קוטבית** $z = r(\\cos\\theta + i\\sin\\theta)$ • כפל: גדלים מוכפלים, זוויות מחוברות',
    '**דה-מואבר** $z^n = r^n(\\cos n\\theta + i\\sin n\\theta)$',
    '**חזקות $i$** $i^1 = i$, $i^2 = -1$, $i^3 = -i$, $i^4 = 1$ • מחזור 4',
