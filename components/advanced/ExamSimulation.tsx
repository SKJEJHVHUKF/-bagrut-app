      ...
      <div className="space-y-3">
        ...
        <div className="surface-premium rounded-2xl p-4 space-y-3">
          <div className="flex items-center gap-2.5">
            <Timer className="w-5 h-5 text-violet-700" />
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        className={`sticky top-16 z-20 backdrop-blur-md border rounded-2xl px-4 py-2.5 flex items-center gap-3 ${
          phase === 'review'
            ...
            : lowTime
              ? 'bg-violet-50/90 border-violet-500/50'
              : 'bg-[var(--background)]/90 border-slate-900/[0.12]'
        }`}
      >
        <Timer className={`w-4 h-4 ${lowTime && phase === 'running' ? 'text-violet-700' : 'text-slate-700'}`} />
        <span
          className={`font-mono text-lg font-black tabular-nums ${
            phase === 'review' ? 'text-slate-600' : lowTime ? 'text-violet-800' : 'text-slate-900'
          }`}
          dir="ltr"
        >
          {mm}:{ss}
        ...
        <div className="flex-1 h-1.5 bg-slate-900/5 rounded-full overflow-hidden">
          <div
            className={`h-full transition-all ${lowTime ? 'bg-violet-500' : 'bg-violet-500'}`}
      ...
      {timedOut && phase === 'review' && (
        <div className="bg-violet-500/10 border border-violet-500/40 rounded-xl px-4 py-2.5 text-sm text-violet-800 font-bold">
        ...
        {q.parts.map((part) => (
          <div key={part.label} className="surface-premium rounded-xl p-3.5 space-y-2.5">
            ...
            <textarea
              ...
              className="w-full bg-slate-900/[0.04] border border-slate-900/10 focus:border-violet-500/60 rounded-xl px-3 py-2 text-sm text-slate-900 placeholder:text-slate-500 outline-none transition-colors resize-y disabled:opacity-60"
            />
              ...
              {phase === 'review' && (
                ...
                >
                  <div className="bg-gradient-to-br from-violet-600/10 to-violet-600/10 border border-violet-500/30 rounded-xl p-3.5">
                    <div className="text-[10px] font-black tracking-widest text-violet-700 mb-2 uppercase">
