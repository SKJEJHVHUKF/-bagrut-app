...
export function WorkedExamView({ exam, index }: { exam: WorkedExam; index: number }) {
  return (
    <div className="bg-slate-900/[0.02] border border-violet-500/20 rounded-2xl overflow-hidden">
      <div className="bg-gradient-to-l from-violet-600/15 to-violet-600/10 border-b border-slate-900/10 px-4 py-3 flex items-center gap-2.5">
        <FileSearch className="w-4 h-4 text-violet-800 flex-shrink-0" />
    ...
    <div className="surface-premium rounded-xl p-3.5 space-y-3">
      ...
      {part.buildsOn && (
        <div className="flex items-start gap-2 border-r-2 border-violet-500/40 pr-3">
          <Link2 className="w-3.5 h-3.5 text-violet-700 flex-shrink-0 mt-0.5" />
          <div className="chat-md text-xs text-slate-700 leading-relaxed flex-1">
            <span className="font-bold text-violet-800">הקשר לסעיפים הקודמים: </span>
      ...
      {part.patternId && (
        <a
          ...
          className="inline-flex items-center gap-1.5 text-[11px] font-bold bg-violet-500/15 border border-violet-500/40 text-violet-800 px-2.5 py-1 rounded-full hover:bg-violet-500/25 transition-colors"
        >
      ...
      {!allShown ? (
        <motion.button
          ...
          className="w-full inline-flex items-center justify-center gap-2 bg-violet-500/15 hover:bg-violet-500/25 border border-violet-500/40 px-4 py-2 rounded-lg font-bold text-violet-800 text-sm transition-colors"
        >
