...
export function GeoFigure({ spec }: { spec: GeoSpec }) {
  ...
  const toXY = (p: Pt): XY => ({ x: PAD + (p[0] - minX) * s, y: PAD + (maxY - p[1]) * s });
  ...
  const labelDir = (n: string): Pt => {
    ...
    for (const m of nb.get(n) ?? []) {
      const q = P[m], d = Math.hypot(q[0] - p[0], q[1] - p[1]) || 1;
      sx += (q[0] - p[0]) / d; sy += (q[1] - p[1]) / d;
    }
    let len = Math.hypot(sx, sy);
    if (len < 0.35) { sx = p[0] - cx; sy = p[1] - cy; len = Math.hypot(sx, sy); if (len < 1e-9) return [0, 1]; return [sx / len, sy / len]; }
    return [-sx / len, -sy / len];
  };
  const unit = (a: XY, b: XY): XY => { const d = Math.hypot(b.x - a.x, b.y - a.y) || 1; return { x: (b.x - a.x) / d, y: (b.y - a.y) / d }; };
    ...
    <div className="my-2 flex justify-center">
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ maxWidth: '100%', height: 'auto' }} role="img" aria-label="סרטוט">
        {polys.map((pg, i) => {
          ...
          return <polygon key={`pg${i}`} points={ns.map((n) => { const q = pt(n); return `${q.x},${q.y}`; }).join(' ')} fill={pg.accent ? FILL_ACCENT : FILL} stroke={pg.accent ? ACCENT : LINE} strokeWidth={pg.accent ? 2.2 : 1.8} strokeLinejoin="round" />;
        })}
        {circles.map((c, i) => {
          const o = pt(c.center);
          return <circle key={`c${i}`} cx={o.x} cy={o.y} r={c.r * s} fill="none" stroke={c.accent ? ACCENT : LINE} strokeWidth={c.accent ? 2.2 : 1.8} strokeDasharray={c.dashed ? '5,4' : undefined} />;
        })}
        {segs.map((sg, i) => {
          const ns = names(sg.s);
          if (ns.length !== 2 || !P[ns[0]] || !P[ns[1]]) return null;
          const a = pt(ns[0]), b = pt(ns[1]);
          return <line key={`s${i}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={sg.accent ? ACCENT : LINE} strokeWidth={sg.accent ? 2.4 : 1.8} strokeDasharray={sg.dashed ? '5,4' : undefined} strokeLinecap="round" />;
        })}
        ...
        {(spec.ticks ?? []).map((t, i) => {
          ...
          return Array.from({ length: n }, (_, k) => {
            ...
            return <line key={`t${i}-${k}`} x1={c.x + u.y * 4.5} y1={c.y - u.x * 4.5} x2={c.x - u.y * 4.5} y2={c.y + u.x * 4.5} stroke={INK} strokeWidth={1.8} />;
          });
        })}
        {(spec.parallel ?? []).map((pr, i) => {
          ...
          return Array.from({ length: n }, (_, k) => {
            ...
            return <polyline key={`p${i}-${k}`} points={`${l.x},${l.y} ${tip.x},${tip.y} ${r.x},${r.y}`} fill="none" stroke={INK} strokeWidth={1.7} strokeLinejoin="round" />;
          });
        })}
        {(spec.right ?? []).map((r, i) => {
          ...
          return <polyline key={`r${i}`} points={`${p1.x},${p1.y} ${p2.x},${p2.y} ${p3.x},${p3.y}`} fill="none" stroke={LINE} strokeWidth={1.5} />;
        })}
        {(spec.angles ?? []).map((an, i) => {
          ...
          const arc = (rad: number) => Array.from({ length: 15 }, (_, k) => {
            const t = ta + (d * k) / 14;
            return `${vs.x + Math.cos(t) * rad},${vs.y - Math.sin(t) * rad}`;
          }).join(' ');
          ...
          return (
            <g key={`a${i}`}>
              ...
              {an.label && (
                <text x={vs.x + Math.cos(mid) * lr} y={vs.y - Math.sin(mid) * lr} fill={col} fontSize={12} fontFamily={FONT} fontWeight={600} textAnchor="middle" dominantBaseline="middle">{an.label}</text>
              )}
            </g>
          );
        })}
        ...
        {(spec.labels ?? []).map((l, i) => {
          ...
          if (l.on) {
            ...
            const crowd = (p: Pt) => Object.entries(P).filter(([n]) => n !== ns[0] && n !== ns[1]).reduce((d, [, q]) => Math.min(d, Math.hypot(q[0] - p[0], q[1] - p[1]) * s), Infinity);
            ...
            if (flip) { nx = -nx; ny = -ny; }
            const q = toXY(m);
            return <text key={`l${i}`} x={q.x + nx * 12} y={q.y - ny * 12} fill={col} fontSize={12.5} fontFamily={FONT} fontWeight={600} textAnchor="middle" dominantBaseline="middle">{l.text}</text>;
          }
          if (l.at && P[l.at]) {
            const q = pt(l.at), dir = labelDir(l.at);
            return <text key={`l${i}`} x={q.x + dir[0] * 32} y={q.y - dir[1] * 32} fill={col} fontSize={12.5} fontFamily={FONT} fontWeight={600} textAnchor="middle" dominantBaseline="middle">{l.text}</text>;
          }
          return null;
        })}
        ...
        {Object.keys(P).filter((n) => !hidden.has(n)).map((n) => {
          ...
          return (
            <g key={`pt${n}`}>
              <circle cx={q.x} cy={q.y} r={2.6} fill={INK} />
              <text x={q.x + dir[0] * 13} y={q.y - dir[1] * 13} fill={INK} fontSize={13.5} fontFamily={FONT} fontWeight={700} textAnchor="middle" dominantBaseline="middle">{n}</text>
            </g>
          );
        })}
      </svg>
