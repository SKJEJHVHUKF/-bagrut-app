    ...
    <section className="mb-5 space-y-3">
      ...
      <div className="flex items-baseline justify-between">
        ...
        <div className="text-xs text-slate-600">
          <span className="font-black text-violet-700">{completed}/3</span>
          <span> שלבים</span>
        </div>
      </div>
      ...
      <div className="h-1.5 bg-slate-900/[0.03] rounded-full overflow-hidden">
        <div
          className="h-full bg-gradient-to-l from-violet-500 to-violet-500 transition-all duration-500"
...
function StepCard({
  ...
  const accent = done
    ...
    : 'border-violet-500/40 bg-gradient-to-br from-violet-600/15 to-violet-600/15 hover:border-violet-500/60';

  const iconBg = done
    ? 'bg-emerald-500/30 text-emerald-800'
    : locked
    ? 'bg-slate-900/[0.03] text-slate-500'
    : 'bg-gradient-to-br from-violet-500 to-violet-500 text-white';
