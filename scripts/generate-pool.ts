...
import { normalizeUnitLevel, GENERATOR_MODEL } from '@/lib/agents/config';
...
async function generateOnce(): Promise<unknown> {
  ...
  // 3500 keeps the headroom this script always had over the production budget
  // (now GENERATOR_MAX_TOKENS = 3200). Truncation here wastes a paid call AND a
  // pool slot, so the extra ceiling is worth more offline than online.
  const maxTokens = 3500;

  const message = await anthropic.messages.create({
    // Same model as the live routes, on purpose: a pool generated by a
    // different model than the fallback path is a pool whose questions read
    // differently from the ones students get on a pool miss.
