...
add({
  id: 'q13d',
  fig: {
    xRange: [-3, 3], yRange: [-0.8, 6.8], halo: true,
    curves: [{ f: f13, from: -1.999, to: 1.999 }],
    vAsym: [{ x: -2 }, { x: 2, label: 'x = 2' }],
    hAsym: [{ y: 1, label: 'y = 1' }],
    guides: [{ x1: -3, y1: 3, x2: 3, y2: 3, color: PINK, dashed: true }],
    points: [{ x: 0, y: 3, label: '(0, 3)', color: INDIGO, dx: 10, dy: 18 }],
    texts: [{ x: 2.55, y: 3.45, text: 'y = 3', color: PINK, bold: true }, { x: -2.1, y: 6.3, text: 'x = -2', color: AMBER, anchor: 'end', bold: true }],
  },
  caption:
    'הענף האמצעי של $f$ עבור $a = 36$, והישר $y = 3$ בוורוד, שנוגע בו רק בנקודת המינימום $(0,\\; 3)$. הקווים הכתומים המקווקווים הם האסימפטוטות $x = -2$, $x = 2$ וגם $y = 1$.',
...
add({
  ...
  fig: {
    ...
    curves: [
      { f: f14, to: -1.001, dashed: true, width: 1.6 },
      { f: f14, from: -0.999, to: 2.999, dashed: true, width: 1.6 },
      { f: f14, from: 3.001, dashed: true, width: 1.6 },
      { f: g14, to: -1.0005, color: PINK }, { f: g14, from: -0.9995, to: -0.0005, color: PINK }, { f: g14, from: 0.0005, to: 2.9995, color: PINK }, { f: g14, from: 3.0005, color: PINK },
    ],
    vAsym: [{ x: 0 }],
    hAsym: [{ y: 1, label: 'y = 1' }],
    points: [
      { x: -3, y: 16 / 9, label: '(-3, 16/9)', color: PINK, dx: -34, dy: -10 },
      { x: -1, y: 0, hollow: true, color: PINK }, { x: 3, y: 0, hollow: true, color: PINK },
    ],
    xTicks: [{ x: -1, label: '-1' }, { x: 3, label: '3' }],
  },
  caption:
    'בוורוד $g(x) = \\dfrac{1}{\\big(f(x)\\big)^2}$, ובכחול מקווקו הגרף של $f$. המינימום של $f$ בגובה $\\dfrac{3}{4}$ הפך למקסימום של $g$ בגובה $\\dfrac{16}{9}$, והאפס של $f$ הפך לאסימפטוטה $x = 0$ של $g$. כאשר $x$ מתקרב אל $-1$ או אל $3$ הפונקציה $f$ שואפת לאינסוף, והגרף של $g$ מתקרב לציר $x$, אך הנקודות $(-1,\\; 0)$ וגם $(3,\\; 0)$ אינן על הגרף.',
  checks: [
...
{
  // a = 3: only the left branch of g is drawn, so part ז's sketch of g is not handed over
  add({
    id: 'q14e',
    fig: {
      xRange: [-6.5, -1.25], yRange: [-0.7, 3.6], halo: true,
      curves: [{ f: g14, from: -6.5, to: -1.25, color: PINK }],
      hAsym: [{ y: 1, label: 'y = 1' }],
      guides: [{ x1: -6.5, y1: 16 / 9, x2: -1.25, y2: 16 / 9, color: PINK, dashed: true }],
      points: [{ x: -3, y: 16 / 9, label: '(-3, 16/9)', color: PINK, dx: -30, dy: -10 }],
      texts: [{ x: -1.9, y: 2.05, text: 'y = 16/9', color: PINK, bold: true }],
      xTicks: [tick(-3)],
    },
    caption:
      'עבור $a = 3$: הענף השמאלי של הגרף של $g$, והישר $y = \\dfrac{16}{9}$ (בוורוד), שנוגע בו בנקודת המקסימום $\\left(-3,\\; \\dfrac{16}{9}\\right)$. בנקודה הזאת המשיק לגרף אופקי. הקו הכתום המקווקו הוא האסימפטוטה $y = 1$.',
    checks: [
      on('g14 max', g14, -3, 16 / 9),
...
add({
  ...
  fig: {
    ...
    points: [
      ...
      { x: 16, y: 1, label: '(16, 1)', color: INDIGO, dx: 8, dy: -14 },
...
add({
  ...
  fig: {
    ...
    vAsym: [{ x: 0, label: 'x = 0' }],
    guides: [{ x1: 0.4, y1: 2 + 0.25 * 3.6, x2: 7.6, y2: 2 - 0.25 * 3.6, color: INK, dashed: true }],
    points: [
  ...
  caption:
    'עבור $a = 8$: בכחול $f(x) = \\sqrt{8 - x}$ ובוורוד $g(x) = \\dfrac{4}{\\sqrt{x}}$. שני הגרפים נוגעים זה בזה בנקודה $(4,\\; 2)$, ובה יש להם משיק משותף בשיפוע $-\\dfrac{1}{4}$ (הקו הכהה המקווקו). בכל נקודה אחרת הגרף של $g$ מעל הגרף של $f$.',
  checks: [
