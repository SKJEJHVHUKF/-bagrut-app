...
import { SPECS, PLACEMENT, SOLUTION_PLACEMENT } from './_rq-figure-specs';
...
// Solution figures are emitted in the same shape, so the same rewrite covers
// them; dedupe because two placements can share one constant.
const ALL = [...new Map([...PLACEMENT, ...SOLUTION_PLACEMENT].map((p) => [p.id, p])).values()];
...
console.log(`${n}/${ALL.length} figures refreshed${DRY ? ' (dry run)' : ''}`);
if (problems.length) {
