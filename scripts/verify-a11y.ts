 ...
 *  whose users are adults with no patience for squinting. Add a path here when
 *  a new staff screen lands; a rule that silently covers nothing is worse than
 *  no rule.
 *
 *  ⚠️ `app/teacher/` is NOT in this list yet, and that is deliberate rather
 *  than an oversight. The private-teacher board still fails these two rules 69
 *  times; its rewrite is a separate change. Adding the path before the screen
 *  is fixed puts a permanently-red gate into `npm run check`, and a gate that
 *  always fails gets bypassed within a day — the same reasoning verify-concept
 *  gives for keeping PROMOTED_TO_ERROR false. Add it in the commit that fixes
 *  the screen, not before. */
const CONSOLE_PATHS = ['app/console/', 'components/console/'];
