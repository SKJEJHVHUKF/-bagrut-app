...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = 2x + 1$. האזור המקווקו שמתחתיו, בין הישר $x = 1$ לבין הישר $x = 3$, הוא טרפז: הבסיסים שלו הם הצלעות האנכיות, באורך $3$ וגם $7$, והגובה שלו, המרחק בין הבסיסים, הוא $2$. שטח הטרפז הוא $10$, ובדיוק זה מה שהאינטגרל $\\int_1^3 (2x+1)\\,dx$ נותן.',
    checks: [
...
// ======== lesson step: חישוב שטח — כל הפרוצדורה
{
  ...
  add({
    ...
    caption:
      'השטח שבין $f(x) = 4-x^2$ לבין ציר $x$. הגבולות הם שיעורי $x$ של נקודות החיתוך עם ציר $x$, $-2$ וגם $2$, וכל האזור נמצא מעל הציר, ולכן אין מה לפצל.',
...
};
{
  ...
  add({
    id: 'ghost006Region',
    fig: { ...BASE006, shade: [R], points: PTS006, texts: LABELS006 },
    caption:
      'האזור הירוק המקווקו כלוא בין הישר, שהוא העליון, לבין הפרבולה, מהערך $-1$ עד הערך $3$. זה האזור שהאינטגרל של ההפרש מחשב.',
    checks: [['area 32/3', areaOf(R), 32 / 3, 1e-7]],
  });
  add({
    id: 'ghost006Meet',
    fig: { ...BASE006, points: PTS006, texts: LABELS006 },
    caption:
      'שני הגרפים נפגשים בשתי נקודות בלבד, $(-1,\\; 1)$ וגם $(3,\\; 9)$. שם האזור שביניהם נפתח ושם הוא נסגר, ולכן שיעורי $x$ שלהן, $-1$ וגם $3$, הם הגבולות.',
  ...
  add({
    ...
    caption:
      'בדיקת הסבירות: רוחב האזור הוא $4$ יחידות, מהערך $-1$ עד הערך $3$, והפער האנכי הגדול ביותר בין הגרפים הוא $4$, כאשר $x = 1$ (הקו השחור המקווקו). מלבן ברוחב $4$ ובגובה $4$ היה מכסה $16$, והאזור המקווקו קטן ממנו.',
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = 3 - \\dfrac{12}{x^2}$ בין הישר $x = 1$ לבין הישר $x = 4$. הוא חוצה את ציר $x$ בנקודה $(2,\\; 0)$: משמאל האזור מתחת לציר, ומימין הוא מעליו. לשני החלקים אותו שטח, $3$, ולכן אינטגרל אחד בלי פיצול מתאפס, והשטח האמיתי הוא $6$. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
    checks: [
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $y = 1 - \\dfrac{2}{\\sqrt{x}}$. בין הישר $x = 1$ לבין הישר $x = 4$ הגרף מתחת לציר $x$, ומגיע אל הציר רק בקצה, בנקודה $(4,\\; 0)$, שבה הוא חוצה אותו. לכן האינטגרל המסוים על האזור המקווקו יוצא שלילי. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = -\\dfrac{4}{x^2}$ נמצא כולו מתחת לציר $x$. האזור המקווקו כלוא בינו לבין הציר, מהישר $x = 1$ עד הישר $x = 2$, ולכן האינטגרל שלילי והשטח הוא הערך המוחלט שלו. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = x - 3\\sqrt{x}$ בין הישר $x = 4$ לבין הישר $x = 16$. הוא חוצה את ציר $x$ בנקודה $(9,\\; 0)$: משמאל האזור מתחת לציר ותורם $\\dfrac{11}{2}$, ומימין הוא מעל הציר ותורם $\\dfrac{27}{2}$.',
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = \\dfrac{2}{x^2}$ עבור $a = 4$. האזור המקווקו כלוא בין הגרף לבין ציר $x$, מהישר $x = 1$ עד הישר $x = 4$, והשטח שלו הוא $\\dfrac{3}{2}$, כנתון. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'בכחול הגרף של $f(x) = \\dfrac{16}{x^2}$, ובוורוד המשיק בנקודה $(2,\\; 4)$, שחותך את ציר $x$ בנקודה $(3,\\; 0)$. האזור המקווקו חסום מלמעלה בגרף, מימין בישר $x = 4$, ומלמטה בשני קטעים: בין $x = 2$ לבין $x = 3$ במשיק, ובין $x = 3$ לבין $x = 4$ בציר $x$. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
    checks: [
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = \\dfrac{8}{x^2}$, ובוורוד מקווקו הישרים $y = 8$ וגם $y = 2$. הם פוגשים את הגרף בנקודות $(1,\\; 8)$ וגם $(2,\\; 2)$, ומהן יורדים האנכים לציר $x$, שהם הגבולות של האזור המקווקו. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = \\dfrac{6}{x^2}$ עבור $a = \\dfrac{4}{5}$. האזור המקווקו כלוא בין הגרף לבין ציר $x$, מהישר $x = \\dfrac{4}{5}$ עד הישר $x = \\dfrac{12}{5}$, והשטח שלו הוא $5$, כנתון. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    id: 'q205',
    fig: {
      xRange: [-1.1, 4.7], yRange: [-0.5, 4.6], curves: [{ f: f205, from: 0.3 }], shade: [R], halo: true,
    ...
    caption:
      'הגרף יורד אל האסימפטוטה האופקית $y = 1$ ואינו חוצה אותה. האזור המקווקו כלוא בין הגרף לבין האסימפטוטה, והקווים הירוקים המקווקווים הם הישרים $x = 1$ וגם $x = 2$ שתוחמים אותו. שני הקווים הכתומים המקווקווים הם האסימפטוטות: האנכית $x = 0$ והאופקית $y = 1$.',
...
}
{
  ...
  add({
    id: 'q206',
    fig: {
      xRange: [-0.6, 4.8], yRange: [-0.6, 4.6], curves: [{ f: f206 }], shade: [S1, S2], halo: true,
      vAsym: [{ x: 0, label: 'x = 0' }],
      xTicks: [tick(1), tick(2), tick(4)], yTicks: [ytick(4), ytick(1)],
      points: [{ x: 1, y: 4, color: INDIGO }, { x: 2, y: 1, color: INDIGO }],
      texts: [t(1.45, 0.8, '2'), t(2.9, 0.22, '1')],
    },
    caption:
      'סעיף ב: הגרף של $f(x) = \\dfrac{4}{x^2}$ ושתי רצועות מקווקוות מתחתיו, והמספר בתוך כל רצועה הוא השטח שלה. הרצועה $S_1$ נמצאת בין $x = 1$ לבין $x = 2$, והרצועה $S_2$ בין $x = 2$ לבין $x = 4$. הרצועה $S_1$ קרובה לאסימפטוטה האנכית $x = 0$, הקו הכתום המקווקו, ולכן היא גבוהה בהרבה: למרות שהיא צרה פי שתיים, השטח שלה, $2$, כפול מזה של $S_2$.',
    checks: [['S1 2', areaOf(S1), 2, 1e-7], ['S2 1', areaOf(S2), 1, 1e-7], on('f206', f206, 2, 1)],
  });
}
{
  ...
  add({
    id: 'q206c',
    fig: {
      xRange: [2.6, 9.2], yRange: [-0.06, 0.34], curves: [{ f: f206, from: 2.6 }], shade: [S3], halo: true,
      xTicks: [tick(4), tick(8)], yTicks: [ytick(0.25)],
      points: [{ x: 4, y: 0.25, color: INDIGO }],
      texts: [t(5.6, 0.05, '0.5')],
    },
    caption:
      'סעיף ג: אותו גרף בהגדלה, בין הישר $x = 4$ לבין הישר $x = 8$, שמתקבל עבור $t = 8$. הרצועה נמוכה אבל ארוכה, והשטח שלה $0.5$.',
    checks: [['part ג 0.5', areaOf(S3), 0.5, 1e-7], on('f206', f206, 4, 0.25)],
  });
}
{
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $f(x) = \\dfrac{x - 4}{\\sqrt{x}}$ בין הישר $x = 1$ לבין הישר $x = 9$. הוא חוצה את ציר $x$ בנקודה $(4,\\; 0)$: משמאל האזור מתחת לציר ותורם $\\dfrac{10}{3}$, ומימין הוא מעל הציר ותורם $\\dfrac{14}{3}$. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'בכחול $f(x) = \\dfrac{9}{x^2}$ ובוורוד $g(x) = 10 - x^2$. הם נפגשים בנקודות $(1,\\; 9)$ וגם $(3,\\; 1)$, ובין שתיהן הפרבולה היא העליונה. האזור המקווקו הוא השטח המבוקש. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'סעיף א: הגרף של $f(x) = \\dfrac{6}{\\sqrt{x}}$, כלומר עבור $a = 6$. האזור המקווקו כלוא בין הגרף לבין ציר $x$, מהישר $x = 1$ עד הישר $x = 9$, והשטח שלו הוא $24$, כנתון. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'סעיף ג: בוורוד מקווקו הישר $y = 3$, שפוגש את הגרף בנקודה $(4,\\; 3)$. האזור המקווקו כלוא בין הישר, שהוא הגבול העליון, לבין הגרף, שהוא הגבול התחתון, עד הישר $x = 9$. הקו הכתום המקווקו הוא האסימפטוטה האנכית $x = 0$.',
...
}
{
  ...
  add({
    ...
    caption:
      'הגרף של $g(x) = 3x^2 - x^3$ בתחום $x \\le 3$: הוא נוגע בציר בראשית, עולה אל המקסימום $(2,\\; 4)$, וחותך את הציר בנקודה $(3,\\; 0)$, שם מסתיים התחום. משמאל לראשית הוא מתרומם ואינו חוזר אל הציר, ולכן האזור הסגור הוא רק החלק המקווקו.',
